...
def milvus_client() -> MilvusClient:
    return MilvusClient(
        ...
        db_name=settings.milvus_db_name or "",
...
def cleanup(namespace_id, index_id, keep_artifacts: bool) -> dict[str, Any]:
    if keep_artifacts:
    ...
    with psycopg.connect(pg_conninfo()) as conn, conn.cursor() as cur:
        cur.execute(
            sql.SQL(
                "select zh_collection_name, en_collection_name "
                "from {}.rag_retrieval_indexes where index_id = %s"
            ).format(sql.Identifier(schema)),
            (index_id,),
        )
        row = cur.fetchone()
        zh_collection_name, en_collection_name = row if row else (None, None)

    client = milvus_client()
    for name in [zh_collection_name, en_collection_name]:
        if name and name in client.list_collections():
